...
This skill runs end-to-end automatically. From a single request, draft the full
memo through all sections without pausing to ask which section to write. Use
inputs the user provided or prior session analysis; where an input is missing,
proceed with **clearly-labeled illustrative assumptions** (mark them
`[ASSUMPTION — confirm]`) rather than stopping. Only stop on a genuine blocker
(e.g., no company specified at all).
...
- When inputs are missing, proceed with clearly-labeled illustrative assumptions (`[ASSUMPTION — confirm]`) on deal terms or returns rather than stopping; summarize all such assumptions at the top so the user can confirm or correct them
