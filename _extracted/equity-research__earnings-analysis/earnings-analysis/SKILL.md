...
**Auto-Run Mode:** Run all 5 phases (Data Collection → Analysis → Charts → Report → QC) end-to-end automatically from a single request. Do not pause between phases to ask the user; only stop on a genuine blocker (e.g., the company or quarter is not specified, or no earnings data can be found).
