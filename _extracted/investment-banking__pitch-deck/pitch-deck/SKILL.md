...
Run all 5 phases (Extract → Map → Populate → Validate/Fix loop → Final
verification) end-to-end automatically once the template and source data are
available. Do not pause between phases to ask the user. The only built-in
stop is the Phase 4 escalation **after 3 fix cycles** if structural issues
genuinely cannot be auto-resolved, and the inherent need for a template +
source data to start. Otherwise proceed automatically.
