...
**Build End-to-End Automatically (self-verify at each stage, do NOT pause for the user):**
- Build the full model in one pass: data retrieval → revenue projections → margin build → FCF schedule → WACC → terminal value + PV → equity bridge → sensitivity tables.
- After each stage, run an internal sanity check before moving on — do NOT stop to ask the user to confirm:
  - After data retrieval → sanity-check the raw inputs block (revenue, margins, shares, net debt vs net cash)
  - After revenue projections → check the projected top line and growth rates are reasonable
  - After FCF build → check the full FCF schedule ties
  - After WACC → check the calculation and CAPM inputs are in range
  - After terminal value + PV → check the equity bridge (EV → equity value → per share) and that TV is 50-70% of EV
- Catch and fix errors at each stage yourself — a wrong margin assumption found after sensitivity tables means fixing it and recalculating downstream automatically.
- Only stop to ask the user on a genuine blocker (e.g., no access to any financial/market data) or if the user explicitly asked to review a stage.
